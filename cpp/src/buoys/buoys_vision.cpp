...
void filterLight(cv::Mat &img)
{
    ...
    cv::Scalar avg_v = cv::mean(channels[2]);
    // Calculate standard deviations
    cv::Scalar dev_v;
    cv::meanStdDev(channels[2], avg_v, dev_v);
    int sdev_v = dev_v[0];

    // Remove everything that is too far from mean
    int max_v = avg_v[0] + sdev_v*DEV_LEVEL;
    for (int row = 0; row < img.rows; row++)
    {
        for (int col = 0; col < img.cols; col++)
        {
            // Get values
            cv::Vec3b channels = img.at<cv::Vec3b> (row, col);
            int h = channels[0];
            int s = channels[1];
            int v = channels[2];
            // Remove bright light
            ...
            channels[1] = s;
            channels[2] = v;
            img.at<cv::Vec3b> (row, col) = channels;
        }
    }
